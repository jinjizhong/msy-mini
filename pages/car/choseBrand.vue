       ...
       <scroll-view scroll-y="true" class="scrollList" scroll-with-animation="true" :scroll-into-view="toView">
            <view class="scroll-item " v-for="(item,index) in dataList" :key="index">
				...
				<view class="scroll-item__group-item" @tap="toChoseCarSys(item)" >
					<image
						class="scroll-item__group-item_img"
						:src="item.picUrl"
						mode="scaleToFill"
						lazy-load="false"
					/>
...
export default {
  ...
  methods: {
	  ...
	  // 选择车系
	  toChoseCarSys(item) {
	      let {brandId,brandName,picUrl} = item
	      wx.navigateTo({ url: `/pages/car/choseCarSys?BrandId=${brandId}&BrandName=${brandName}&BrandImage=${picUrl}` });
	  },
	  touchEnd (e) {
	  ...
	  async getData() {
		  let dataList = await this.$store.dispatch('getVehicleBrand');
