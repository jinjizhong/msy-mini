            ...
            <view class="scroll-item " v-for="(item,index) in dataList" :key="index">
				...
				<view class="scroll-item__group-item" @tap="toChoseCarSys(item)" >
					<view class="scroll-item__group-item_name">
						{{item.vehicleSeriesName}} {{item.description}}
					</view>
				</view>
                <!-- </view> -->
            </view>
...
export default {
  ...
  methods: {
	  ...
	  // 选择车系
	  toChoseCarSys(item) {
	      let {BrandId,VehicleSeriesName,Description:VehicleTypeName,vehicleSeriesName} = item
		  let {PageOption:{BrandName,Description,BrandImage}} = this
		  let carVehicleTypeName = `${VehicleTypeName} ${vehicleSeriesName}`
		  uni.$emit("storeListen",{BrandName,VehicleSeriesName:Description,carVehicleTypeName,BrandImage})
		  uni.navigateBack({
			 delta: 3
		  })
		},
	  ...
	  async getData(options) {
		  let params ={
			  ...
			  vehicleSeriesName:options.VehicleSeriesName
		  }
		  let dataList = await this.$store.dispatch('getVehicleType',params);
